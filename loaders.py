...
from coreferee.rules import RulesAnalyzer
...
class DEMOCRATConllLoader(GenericLoader):
    ...
    def turn_spans_to_mentions(self, coreference_spans:dict, j=0,txt='') :
        mentions = {}
                ...
                if reference.startswith('(') and reference.endswith(')'):
                    ...
                    for browsed_mention_span, browsed_mention_label in reversed(mentions.items()):
                        if browsed_mention_label == mention_label and isinstance(browsed_mention_span[1],str):
                            ...
                            if new_mention_span in mentions :
    ...
    def load(self, directory_name:str, nlp:Language, rules_analyzer:RulesAnalyzer) -> list:
        ...
        docs = nlp.pipe(txt_file_contents)
        #assert len(list(docs)) == len(doc_mentions_spans) == len(txt_file_contents)
        #print(len(list(docs)) , len(doc_mentions_spans) , len(txt_file_contents))
        docs_to_return = []
        for index, doc in enumerate(docs):
            if index % 10 == 0:
                print('Loaded', index, 'documents')
...
if __name__ == '__main__':
    ...
    docs = loader.load('d:/Utilisateurs/souma/Documents/Projets_Perso/Coreference/corpus/training_french_corpus',
            nlp=nlp,rules_analyzer=rules_analyzer,verbose=True)
    print(len(docs))